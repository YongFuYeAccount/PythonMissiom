# -*- coding: utf-8 -*-
"""
Created on Wed Oct 20 21:31:03 2021
...
def SERVER1():
    ...
    port = 9003
    s.bind((host, port))#表示TCP服务器的套接字地址
    s.listen(5)
    while True:
        print('等待TCP客户端连接……')
        c, addr = s.accept()        #阻塞进程，等待客户端接入
        print('TCP客户端已连接 address:{}\n服务器:请发送数据'.format(addr))
        c.send(bytes("请问需要什么帮助?",encoding="utf-8"))
        while True:
            mes = str(c.recv(1024),encoding = "utf-8")
            print("客户端发送的数据是：{}".format(mes))

            while True:
                time.sleep(5)#时间间隔
                #msg = f"messages: {msg_count}"#源程序中发布的消息
                msg = mes   #将tcp客户端发送的信息作为emq订阅端的发布消息
                result = connect_mqtt().publish(topic, msg)#调用发布函数发布主题消息
                # result: [0, 1]
                status = result[0]
                if status == 0:
                    print(f"Send message：`{msg}` to topic `{topic}`")#打印发布的消息
                else:
                    print(f"Failed to send message to topic {topic}")#发布失败
